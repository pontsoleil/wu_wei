...
(function (ns) {
  ...
  function isEnabled(note) {
    ...
    return !!(target && (target.joint && target.joint.enabled) || (target.collaboration && target.collaboration.enabled));
  }
  ...
  function originOf(note) {
    var target = note || currentNote();
    return (target && target.origin && typeof target.origin === 'object') ? target.origin : {};
  }

  function originTypeOf(note) {
    return String(originOf(note).type || '').toLowerCase();
  }

  function originSourceOf(note) {
    return String(originOf(note).source || '').toLowerCase();
  }

  function isImportOrigin(note) {
    var type = originTypeOf(note);
    var source = originSourceOf(note);
    return type === 'import' || source === 'export-package';
  }

  function isTeamOrigin(note) {
    var type = originTypeOf(note);
    var source = originSourceOf(note);
  ...
  function isImportedNote(note) {
    ...
    var noteState = String(target && (target.jointNoteState || target.collabNoteState) || '').toLowerCase();

    if (noteState === 'imported') {
      return true;
    }
    if (noteState === 'team' || noteState === 'own') {
      return false;
    }
    if (isImportOrigin(target)) {
      return true;
    }
  ...
  function isTeamNote(note) {
    ...
    if (String(target && target.note_scope || '').toLowerCase() === 'team' ||
      isEnabled(target) ||
      isTeamOrigin(target) ||
      (target && target.team_id)) {
      return true;
    }
    return false;
  }
  ...
  function start(note) {
    var target = note || currentNote();
    if (target) {
      target.joint = normalizeMetadata(target.joint || target.collaboration);
      target.joint.enabled = true;
      target.joint.updatedAt = new Date().toISOString();
      target.jointNoteState = 'team';
      target.note_scope = 'team';
      target.origin = Object.assign({}, target.origin || {}, {
        type: 'team',
        source: 'team-note'
      });
    }
  ...
  function getRevision(note) {
    return normalizeMetadata((note || currentNote()).joint || (note || currentNote()).collaboration).revision;
  }
  ...
  function canEditObject(record) {
    ...
    try {
      ...
      console.warn('joint note edit policy rejected record:', e);
      return false;
    }
  }
