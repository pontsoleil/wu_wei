/**
 * info.viewpoint.js
 * Viewpoint axis and PageMarker info controller
 */
...
(function (ns) {
  ...
  function ensureInfoRoot() {
    ...
    /*
     * Viewpoint info can also be opened directly from the context-menu [i]
    ...
    if (wuwei.info && wuwei.info.markup && typeof wuwei.info.markup.template === 'function') {
      if (!hasHeader || !document.getElementById('info-viewpoint')) {
  ...
  function ensurePane() {
    ...
    var pane = document.getElementById('info-viewpoint');
    if (pane) {
      hideSiblingPanes();
      return pane;
    }
    if (!infoPane || !wuwei.info.viewpoint.markup) { return null; }
    infoPane.insertAdjacentHTML('beforeend', wuwei.info.viewpoint.markup.paneTemplate());
    hideSiblingPanes();
    return document.getElementById('info-viewpoint');
  }
  ...
  function collectMarkerRows(spec) {
    ...
    rows = group.members.map(function (member) {
      ...
      return {
        ...
        viewerUri: documentNode && viewpoint && typeof viewpoint.getContentTargetViewerUrl === 'function'
          ? viewpoint.getContentTargetViewerUrl(documentNode, pageNumber, point)
          : ''
      };
    }).filter(Boolean);
  ...
  function openAxis(groupOrTarget) {
    ...
    var pageOffset = wuwei.viewpoint && typeof wuwei.viewpoint.getPageNumberOffset === 'function'
      ? wuwei.viewpoint.getPageNumberOffset(group)
      : 0;

    pane.innerHTML = wuwei.info.viewpoint.markup.axisTemplate({
      group: group,
  ...
  function openMarker(pointOrTarget) {
    ...
    pane.innerHTML = wuwei.info.viewpoint.markup.markerTemplate({
      point: spec.point,
      axisName: spec.group ? (spec.group.name || spec.group.id) : '',
      documentName: getDocumentName(spec),
      pageNumber: spec.pageNumber,
      viewerUri: spec.documentNode && viewpoint && typeof viewpoint.getContentTargetViewerUrl === 'function'
        ? viewpoint.getContentTargetViewerUrl(spec.documentNode, spec.pageNumber, spec.point)
        : ''
    });
    pane.style.display = 'block';
    return true;
  ...
  function open(target, option) {
    var point = option && (option.displayedContentTarget || option.contentTarget || option.contentTargetPoint || option.displayedPageMarker || option.viewpointPoint);
    var spec;
  ...
  function openViewpointAxisInInfo(group) {
    ensureInfoRoot();
