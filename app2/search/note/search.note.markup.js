/**
 * search.note.markup.js
 * wuwei search.note template
 *
 ...
 **/
wuwei.search.note.markup = ( function () {
  let _files;
  
  const template = function (files) {
    if (files) {
      _files = files;
    } else {
      _files = [];
      ...
      if (format.indexOf('image') >= 0 || 'application/pdf' === format) {
        html = `
          <img src="${file.value.thumbnail.uri}" class="w3-col s3" id="${file.id}"
              ...
              onclick="wuwei.search.note.showInfo('${file.id}')">`;
      } else if (format && (
...
<ul class="list-group">
  ...
  `<li class="list-item w3-row">
    ...
    <div class="action w3-col s1">
      <a>
        <i id="${file.id}"
          class="fas fa-plus"
          onclick="wuwei.search.note.noterecord('${file.id}')">
        </i>
      </a>
      <a>
        <i class="fas fa-info"
  ...
  function add_calendar(year, month) {
    const wrapper = document.querySelector('#search-note .calendar');
