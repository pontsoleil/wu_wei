...
var PDFViewerApplication = {
  ...
  wuweiInitialPage: (function () {
    var match = (document.location.search || '').match(/[?&]page=([0-9]+)/) ||
      (document.location.hash || '').match(/(?:^#|[&#])page=([0-9]+)/);
    var page = match ? Math.floor(Number(match[1] || 1)) : 0;
  ...
  applyWuweiInitialPage: function pdfViewApplyWuweiInitialPage() {
    var page;
    if (!this.wuweiInitialPage || !this.pdfViewer) {
      return;
    }
    page = Math.max(1, Math.floor(Number(this.wuweiInitialPage || 1)));
    if (this.pdfDocument && this.pdfDocument.numPages) {
      page = Math.min(page, this.pdfDocument.numPages);
    }
    this.scrollWuweiPageIntoView(page);
    var self = this;
    setTimeout(function () {
      self.scrollWuweiPageIntoView(page);
    }, 100);
    setTimeout(function () {
      self.scrollWuweiPageIntoView(page);
    }, 400);
  ...
  scrollWuweiPageIntoView: function pdfViewScrollWuweiPageIntoView(page) {
    var container = document.getElementById('viewerContainer');
    var pageEl = document.getElementById('pageContainer' + page);
    if (!container || !pageEl) {
      return;
    }
    this.page = page;
    if (this.pdfViewer && this.pdfViewer.scrollPageIntoView) {
...
//#endif
  ...
  load: function pdfViewLoad(pdfDocument, scale) {
    ...
    Promise.all([firstPagePromise, storePromise, showPreviousViewOnLoadPromise,
      ...
      var storedHash = null;
      if (self.wuweiInitialPage) {
        storedHash = 'page=' + self.wuweiInitialPage;
      } else if (showPreviousViewOnLoad && store.get('exists', false)) {
  ...
  setHash: function pdfViewSetHash(hash) {
    ...
    if (hash.indexOf('=') >= 0) {
      ...
      if (dest) {
        ...
        this.scrollWuweiPageIntoView(pageNumber);
        setTimeout(function () {
          PDFViewerApplication.pdfViewer.scrollPageIntoView(pageNumber);
          PDFViewerApplication.scrollWuweiPageIntoView(pageNumber);
        }, 0);
      }
      ...
      this.page = hash;
      this.pdfViewer.scrollPageIntoView(Number(hash));
      this.scrollWuweiPageIntoView(Number(hash));
      setTimeout(function () {
        PDFViewerApplication.pdfViewer.scrollPageIntoView(Number(hash));
        PDFViewerApplication.scrollWuweiPageIntoView(Number(hash));
      }, 0);
...
//#if (GENERIC || B2G)
  ...
  var filePageMatch = String(file || '').match(/[?&]page=([0-9]+)(?:&|$)/);
  if (filePageMatch) {
    PDFViewerApplication.wuweiInitialPage =
      Math.max(1, Math.floor(Number(filePageMatch[1] || 1)));
    file = String(file || '')
      .replace(/([?&])page=[0-9]+(?=&|$)/, function (all, prefix) {
        return prefix === '?' ? '?' : '';
      })
      .replace(/[?&]$/, '');
  }
...
//#endif
  ...
  // WuWei serves uploaded PDFs as local managed files. This legacy PDF.js build
  // can leave missing byte ranges in modern browsers, so load the full file.
