    ...
    intersectEllipseEllipse: function (c1, rx1, ry1, c2, rx2, ry2) {
        ...
        if (pts.length == 8) {
            ...
            if (
                (yRoots.length == 2) ||
                (yRoots.length == 4 && 
                    (rootPointsN[0] == 2 && rootPointsN[1] == 2 ||
                     rootPointsN[2] == 2 && rootPointsN[3] == 2))
            ) {
                pts.splice(2, 2);
            }
        }
...
function intersect(shape1, shape2, m1, m2) {
    ...
    if (ip1 !== null && ip2 !== null) {
        var method;
        method = composedShapeMethods[ip1.type];
        if (method) {
            result = method(ip1, ip2, m1, m2);
        }
        else {
            method = composedShapeMethods[ip2.type];
            if (method) {
                ...
                var m1_, m2_;
                var d2;
                var c1, c2, rx1, ry1, rx2, ry2;
                if (useCTM) {
                    if (type1 === IPTYPE.ELLIPSE && type2 === IPTYPE.ELLIPSE) {
                        c1 = params1[0]; rx1 = params1[1]; ry1 = params1[2];
                        c2 = params2[0]; rx2 = params2[1]; ry2 = params2[2];
