...
現在の実装では、アップロード実体、Resource library entry、ノートを分けて保存する。
...
`/wu_wei2/data/` は直接アクセスを禁止する。`load-file.cgi` が認証とパス検査を行った後、必要に応じて `/_wuwei2_data/` の internal location を経由してファイルを返す。
