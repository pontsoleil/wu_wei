...
# -*- coding: utf-8 -*-
"""Load an app ver0 WuWei note.

This script returns only the decoded JSON body contained in the ver0 note
...
def note_file_from_key(root: Path, note_key: str, note_id: str) -> Path | None:
    ...
    if note_id and candidate.name != note_id:
        meta = read_ver0_note_meta(candidate)
...
def note_candidates_by_id(root: Path, note_id: str) -> list[Path]:
    ...
    for path in list_ver0_note_files(root, include_new_note=True):
        if path.name == note_id:
            candidates.append(path)
            continue
        try:
            meta = read_ver0_note_meta(path)
        except Exception:
            meta = {}
        if trim(meta.get("id", "")) == note_id:
            candidates.append(path)
...
def resolve_selected_v0_note_file(root: Path, note_id: str, note_key: str) -> Path:
...
def main() -> None:
    debug("script begin", name="load-note-v0")
    params = merge_query_and_body_params()
    debug_kv(name="load-note-v0", params=params)
    ...
    root = Path(note_dir)
    file_path = resolve_selected_v0_note_file(root, note_id, note_key)
    debug_kv(name="load-note-v0", file_path=str(file_path))

    if not file_path.exists() or not file_path.is_file():
        script_error("ERROR VER0 NOTE FILE NOT FOUND")
    if file_path.name == "note.json":
        script_error("ERROR NOT VER0 NOTE")
    if not is_relative_to(file_path, root):
        script_error("ERROR INVALID NOTE PATH")
    if not is_ver0_note_file(file_path):
        script_error("ERROR NOT VER0 NOTE")
...
if __name__ == "__main__":
    try:
        ...
        debug_exception(name="load-note-v0")
        raise
